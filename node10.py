...
def process_request_packets(msg):
    ...
    # neibors_cluster.update({node_id: {'position': position, 'packets_id': list(packets)}})
    ...
    if local_need == set():
        ...
        if(all_unique_packets != set()):
            ...
            if need_packets >= all_unique_packets:
                ...
                if NodeAndUniqPck[node_id] != []:
                    ...
                    print(f"  - Мои оставшиеся (не уникальные) пакеты: {list(remaining_packets & set(neibors_cluster[node_id]['packets_id']))}")
                    # Наполняем сообщение пакетами
                    for i in list(remaining_packets & set(neibors_cluster[node_id]['packets_id'])):
                        msg_to_send[i] = packets[i] # {packet_idx: value}
        ...
        print(f"  - Соседи {local_stok} НЕ соержат необходимые пакеты\n  - Не хватает: {local_need}")
...
def retranslation(msg, mode):
    print("\nЗапускаю алгоритм ретрансляции мета-данных")
    ...
    while True:        
        if is_sink and temp_data != {}: # Пришла информация о существоании в сети некоторого изображения (индексы его пакетов)
            ...
            if need_packets != set() and network_status[node_id]['neibors'] != {}:        
                ...
                send_in(msg_type='Request', data=msg_request_packets)
